...
visualize <- function(x,main){
  if (is.factor(x)){ barplot(table(x), col='grey', main=main)
  } else { hist(x, col='grey', main=main, xlab='', breaks=25) }
}
...
# 조금 더 상세히 살펴보기 위하여 상기 변수 기반 유형별 경험적 밀도/누적분포 시각화 함수 정의
dens <- function(x){
  par(mfrow=c(length(x),2))
  for (i in 1:length(x)){
    plot(density(x[[i]]), main= paste('Density_',names(x)[i],sep=''),xlab='',
         xlim=c(min(dat$charges), max(dat$charges)))
...
# 나이
dat_age <- split(dat$charges, dat$age)
dens(dat_age)

# BMI
dat_bmi <- split(dat$charges, dat$bmi)
dens(dat_bmi)
...
for (i in 1:(ncol(dat)-2)){
  for (j in (i+1):(ncol(dat)-1)){
    ...
    if (chi_test$p.value < 0.05) {
    cat(paste('##############', colnames(dat)[i],'&',colnames(dat)[j],'##############\n'))
    print(chi_test)
    }
  }
}
